...
IF NOT EXISTS (SELECT 1 FROM sys.indexes 
               WHERE name = 'IX_Pagos_UF_Fecha'
                 AND object_id = OBJECT_ID('bda.Pagos'))
BEGIN
    CREATE NONCLUSTERED INDEX IX_Pagos_UF_Fecha
        ON bda.Pagos (id_unidad, fecha_pago)
        INCLUDE (importe, asociado, cta_origen);
END;
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes 
               WHERE name = 'IX_Expensa_Consorcio_Mes'
                 AND object_id = OBJECT_ID('bda.Expensa'))
BEGIN
    CREATE NONCLUSTERED INDEX IX_Expensa_Consorcio_Mes
        ON bda.Expensa (id_consorcio, mes, fecha_emision, vencimiento2);
END;
GO